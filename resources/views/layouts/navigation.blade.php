...
<aside :class="{ '-translate-x-full': !sidebarOpen, 'translate-x-0': sidebarOpen }"
    class="fixed left-0 top-0 z-40 h-screen w-64 border-r border-gray-200 bg-white shadow-sm transition-transform duration-200 ease-in-out sm:translate-x-0"
    aria-label="Sidebar">
    <div class="flex h-full flex-col">
        <div class="flex h-16 shrink-0 items-center justify-between gap-2 border-b border-gray-200 px-4 sm:px-6">
            ...
            <button type="button" @click="sidebarOpen = false"
                class="rounded-lg p-2 text-gray-500 hover:bg-gray-100 sm:hidden" aria-label="Tutup menu">
        ...
        <div class="border-t border-gray-200 p-3">
            <div class="mb-2 px-3 py-2 text-xs font-medium text-gray-500">{{ Auth::user()->name }}</div>
            <x-nav-link href="/profile" :active="request()->routeIs('profile.edit')" variant="sidebar">
                <svg class="h-5 w-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <form method="POST" action="{{ route('logout') }}" class="block">
                @csrf
                <button type="submit"
                    class="flex w-full items-center gap-3 rounded-lg px-3 py-2 text-sm font-medium text-gray-700 hover:bg-gray-100 hover:text-gray-900">
                    <svg class="h-5 w-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
